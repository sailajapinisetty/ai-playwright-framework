...
test('Search for a speaker using a valid name returns matching results', async ({ page }, testInfo) => {
  try {
    await page.goto('/demo_app/');

    // Navigate to the demo application homepage
    await page.goto('/demo_app/');

    // Wait for the search input field to be visible
    await waitForWithFallback(page, `[data-testid='search-input']`, 'Wait for the search input field to be visible');

    // Click the search input to focus it
    await clickWithFallback(page, `[data-testid='search-input']`, 'Click the search input to focus it');

    // Type 'John' into the speaker search input field
    await fillWithFallback(page, `[data-testid='search-input']`, `John`, 'Type \'John\' into the speaker search input field');

    // Confirm the search input is visible after typing
    await expectVisibleWithFallback(page, `[data-testid='search-input']`, 'Confirm the search input is visible after typing');

    // Submit the search query by pressing Enter
    await pressWithFallback(page, `[data-testid='search-input']`, `Enter`, 'Submit the search query by pressing Enter');

    // Wait for the search results container to appear after submitting
    await waitForWithFallback(page, `[data-testid='search-results']`, 'Wait for the search results container to appear after submitting');

    // Verify the search results container is visible on the page
    await expectVisibleWithFallback(page, `[data-testid='search-results']`, 'Verify the search results container is visible on the page');

    // Verify at least one speaker card result is rendered
    await expectVisibleWithFallback(page, `[data-testid='speaker-card']`, 'Verify at least one speaker card result is rendered');

    // Verify the first speaker card contains the searched name 'John'
    await expectTextWithFallback(page, `[data-testid='speaker-card']`, `John`, 'Verify the first speaker card contains the searched name \'John\'');

  } finally {
    if (!page.isClosed()) {
      await page.screenshot({ path: 'generated_tests/user_story_1-cli-input/screenshots/tc-001/final-ui-tc-001.png', fullPage: true });
      await page.screenshot({ path: `test-results/user_story_1_search_for_a_speaker_using_a__${testInfo.project.name}.png`, fullPage: true });
    }
  }
});